...
const repeatMyParam = () => {
    //YOUR CODE HERE
    return axios.get('http://localhost:3000/repeat/tonga')
        .then(res => {
            let repeatData = document.getElementById('repeat-text')
            repeatData.textContent = res.data
            console.log(repeatData)
            repeatData.style.display = 'block'
        })
}
...
// CODE HERE
const query = () => {
    axios.get('http://localhost:3000/query-test?marty-robbins=became-a-runnin-gun&&spencer=tonga')
    .then(({data}) => {
        console.log(data)
...
//two query = Sent more than two queries and created an object
...
const createFood = (e) => {
    ...
    axios.post(`http://localhost:3000/food`, body)
        .then(res => {
            let foodData = document.createElement('p')
                foodData.textContent = res.data
                document.querySelector('body').appendChild(foodData)
        })
}
